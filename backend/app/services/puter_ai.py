...
class PuterAI(BaseChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        model_val = model or getattr(settings, "PUTER_MODEL", "gpt-5.4-nano")
        token = auth_token or getattr(settings, "PUTER_AUTH_TOKEN", None) or os.getenv("PUTER_AUTH_TOKEN", "")
        super().__init__(
            model=model_val,
            auth_token=token,
    ...
    def _build_payload(self, messages: List[BaseMessage], **kwargs: Any) -> tuple[Dict[str, str], Dict[str, Any]]:
        token = self.auth_token or getattr(settings, "PUTER_AUTH_TOKEN", None) or os.getenv("PUTER_AUTH_TOKEN", "")
        if not token:
            raise ValueError(
...
def puter_chat(prompt: str, model: Optional[str] = None, **kwargs: Any) -> str:
...
async def async_puter_chat(prompt: str, model: Optional[str] = None, **kwargs: Any) -> str:
    ai = PuterAI(model=model)
